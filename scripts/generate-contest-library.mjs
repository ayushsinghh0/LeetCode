...
for (const [tag, rule] of Object.entries(patternMap.direct)) {
  ...
  for (const p of rule.patterns ?? []) {
    if (!PATTERN_IDS.has(p)) fail(`direct["${tag}"] maps to unknown pattern "${p}"`);
  }
...
for (const [i, combo] of patternMap.combinations.entries()) {
  ...
  for (const p of combo.patterns ?? []) {
    if (!PATTERN_IDS.has(p)) fail(`combinations[${i}] maps to unknown pattern "${p}"`);
  }
...
/**
 * Resolve AICM patterns for a contest-only problem from its LeetCode tags.
 *
