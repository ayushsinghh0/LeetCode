...
import { Disclosure, Ledger, Meta, Page, PageColumns, PageHeader, RuledItem, RuledList, Section } from '@/components/layout/Page';
  ...
  return (
    <Page>
      ...
      >
        ...
        {/* The remaining findings, directly under the lead they follow from. They used to be
            filed under question five, ~2,600px down — every figure on the page came before them,
            on a page whose own docstring says findings before figures. No heading over the list:
            it already names itself ("Other findings"), and a rung of heading machinery here would
            only push the first question further down. */}
        <InsightList insights={secondaryInsights} />

        {restInsights.length > 0 && (
          <Disclosure
            summary="More findings"
            meta={`${restInsights.length} more`}
