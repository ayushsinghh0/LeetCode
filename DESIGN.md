...
**Fit by composition, never by clipping (V12.1).** The way a Screen-class route gets shorter is density, not height locks: merge a caption into its bar's row (`DailyGoalProgress dense`), put a section's one control in its header's `action` slot (Today's capacity chips), read a row on one baseline instead of three bands (Roadmap's day rows), fold a long list behind an in-list "Show N more · ~Xm" row (SessionPlan past 6 items), and let `lg:min-h-9` relax the 44px touch floor where the pointer is fine (WCAG 2.5.8's 24px holds). Budget: the first ~590px of viewport must carry the route's whole decision layer; whatever scrolls below it is depth, and it scrolls in `main` like everything else.
...
Because exactly one is ever active, the row is a **`role="radiogroup"` with `aria-checked`**, not a set of independent `aria-pressed` toggles — six toggle buttons announce six independent on/off controls for what is one choice, and arrow-key selection is the contract the radio role promises. Today and Revision both carry this; they are the same setting, so they are the same control. On Today the row sits in the plan section header's `action` slot (the budget is that section's one control); below `sm` it wraps under the title, where it stood as a row.
