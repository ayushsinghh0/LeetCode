...
Deliberately deferred (extension seams designed, not fabricated): course spaced-repetition/flashcards/quizzes/interview banks (would require inventing lecture content; `CourseWeekProgress` can grow `revisionStage/nextRevision` exactly like questions when real prompts exist), knowledge-graph page, course achievements (ACHIEVEMENTS list + ctx is a locked seam; additive later), global-search course results (SearchDialog is question-shaped; course lives on one scannable page), and heatmap/streak inclusion — course XP flows into the day ledger, but `hasActivity()` stays a DSA-practice metric so the locked streak spec is untouched. No artificial week-locking: the syllabus de-emphasizes done weeks and highlights the current one instead (no glow — design system).
