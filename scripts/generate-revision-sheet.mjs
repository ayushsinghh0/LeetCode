...
// Hand-verified external links (master plan T1.13): `{platform, title, url, verifiedAt}[]`.
// Ships empty — entries are maintainer work, each checked by hand before it is added. An entry
// that matches no external row is a fabrication and fails the build; unlisted rows stay unlinked.
const externalLinks = read('scripts', 'data', 'external-links.json');
...
for (const r of resolved.sheet) {
  ...
  if (r.status === 'leetcode') {
    ...
    // Named, never guessed. A hand-verified entry in external-links.json may supply a URL
    // (matched below, after the walk); everything unlisted stays unlinked — a fabricated URL is
...
/* ── Verified external links (T1.13) ─────────────────────────────────────────────────────── */

// Closed world, like everything else here: an entry must be https, name a platform the sheet
// itself uses, match an external row exactly by (platform, title) — and must never name an
// LC-resolved title (a LeetCode problem's link comes from its own universe, never this table).
let linkedExternalRows = 0;
{
  if (!Array.isArray(externalLinks)) fail('external-links.json must be an array');
  const lcTitles = new Set(resolved.sheet.filter((r) => r.status === 'leetcode').map((r) => r.title));
  for (const entry of Array.isArray(externalLinks) ? externalLinks : []) {
    const where = `external-links "${entry?.title}"`;
    if (typeof entry?.platform !== 'string' || typeof entry?.title !== 'string') {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(entry.verifiedAt ?? '')) fail(`${where}: verifiedAt must be yyyy-MM-dd`);
    if (!platformIdx.has(entry.platform)) {
    ...
    const pIdx = platformIdx.get(entry.platform);
    // Attach the URL to every matching external row (a repeated row is linked everywhere).
    let matched = 0;
    for (const row of rows) {
      if (row[1] === 3 && row[2] === entry.title && row[4] === pIdx) {
        row.push(entry.url);
        matched++;
      }
    }
...
const out = {
  _readme:
    ...
    'verifiedUrl?] (the optional 6th column is a hand-verified https link from ' +
    'scripts/data/external-links.json; unlisted rows stay unlinked), ' +
...
console.log(`  external rows        ${String(stats.externalRows).padStart(5)}  (platform named; ${linkedExternalRows} carry a hand-verified link)`);
